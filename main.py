#!/usr/bin/env python3
"""
Run recommendation prompts (no personas) against a model using vLLM.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run recommendation questions (no personas)")
    ...
    p.add_argument("--seed", type=int, default=12345, help="Seed (SamplingParams.seed)")
...
def main() -> None:
    ...
    if args.scale_for_model_size:
        m = re.search(r"(\d+(?:\.\d+)?)[Bb]\b", model_id)
        if m:
    ...
    try:
        for seed in seeds:
            ...
            results = llm.run_batch(prompts, sampling, output_field="response")
